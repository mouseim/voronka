...
export function StartScreen(props: StartScreenProps) {
  ...
  const serverOnly = useMemo(() => props.serverFunnels.filter((funnel) => {
    const serverStatus = funnel.publishedAt ? 'published' : 'draft'
    return !localFunnelIds.has(funnel.id)
      && (status === 'all' || status === serverStatus)
      && funnel.name.toLowerCase().includes(query.trim().toLowerCase())
  }), [localFunnelIds, props.serverFunnels, query, status])
    ...
    <div className="home-page" onDragEnter={(event) => { if (event.dataTransfer.types.includes('Files')) setDragging(true) }}>
      ...
      <main className="home-content">
        ...
        <section className="primary-actions">
          ...
          <button className="action-card open" onClick={() => setConnectionOpen(true)}><span className="action-icon"><Cloud size={25} /></span><span><strong>Подключить мои воронки</strong><small>Загрузить проекты с сервера</small></span><i>→</i></button>
      ...
      <footer className="home-footer"><span>Черновики сохраняются на этом устройстве</span><span>Серверные черновики и опубликованные воронки доступны после подключения</span></footer>
...
function ServerDraftCard({ funnel, onOpen, onDelete }: { funnel: ServerFunnelSummary; onOpen: (funnel: ServerFunnelSummary) => void; onDelete: StartScreenProps['onDelete'] }) {
  const published = Boolean(funnel.publishedAt)
  return <article className="draft-card server-card"><div className="draft-card__top"><span className="draft-file-icon"><Cloud size={21} /></span><span className={`draft-status ${published ? 'published' : 'draft'}`}>{published ? 'Опубликована' : 'Черновик'}</span><span className="version-pill">v{funnel.activeVersion}</span></div><h3>{funnel.name}</h3><div className="draft-location"><span className="server">На сервере{funnel.isDefault ? ' · основная' : ''}</span></div><div className="draft-health">{published ? <span className="ok"><CheckCircle2 size={13} /> Готова к загрузке</span> : <span><Cloud size={13} /> Серверный черновик</span>}</div><div className="draft-meta"><span><Blocks size={14} /> {funnel.nodeCount} {plural(funnel.nodeCount, ['блок', 'блока', 'блоков'])}</span><span><Clock3 size={14} /> {formatRelative(funnel.updatedAt)}</span></div><div className="draft-card__actions"><button className="button primary" onClick={() => onOpen(funnel)}>Открыть</button></div><button className="text-button danger funnel-delete" onClick={() => onDelete({ funnelId: funnel.id, name: funnel.name, onServer: true })}><Trash2 size={14} /> Удалить воронку</button></article>
}

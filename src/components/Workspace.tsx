...
import { checkYooKassa, getYooKassaStatus, integrationConnection, saveYooKassa, setIntegrationConnection, type YooKassaIntegrationStatus } from '../services/integrations'
...
function ResultButtons({ buttons, products, onChange }: { buttons: ResultButton[]; products: Product[]; onChange: (buttons: ResultButton[]) => void }) {
  const patchButton = (id: string, changes: Partial<ResultButton>) => onChange(buttons.map((item) => item.id === id ? { ...item, ...changes } : item))
  return <div><div className="section-label"><span>Кнопки после результата</span><b>{buttons.length}</b></div>{buttons.map((button) => <div key={button.id}><div className="result-button-row"><Field label={button.abText !== undefined ? 'Вариант A' : 'Текст кнопки'}><input value={button.text} onChange={(event) => patchButton(button.id, { text: event.target.value })} /></Field><select value={button.action} onChange={(event) => patchButton(button.id, { action: event.target.value as ResultButton['action'] })}><option value="branch">Следующая ветка</option><option value="url">Ссылка</option><option value="product">Продукт</option></select>{button.action === 'product' && <select value={button.productId ?? ''} onChange={(event) => patchButton(button.id, { productId: event.target.value })}>{products.map((product) => <option value={product.id} key={product.id}>{product.name}</option>)}</select>}<button className="text-button" onClick={() => button.abText === undefined ? patchButton(button.id, { abText: '' }) : patchButton(button.id, { abText: undefined })}>{button.abText === undefined ? 'A/B' : 'Отключить A/B'}</button><button className="mini-icon danger" onClick={() => onChange(buttons.filter((item) => item.id !== button.id))}><Trash2 size={14} /></button></div>{button.abText !== undefined && <Field label="Вариант B"><input value={button.abText} placeholder="Узнать подробнее" onChange={(event) => patchButton(button.id, { abText: event.target.value })} /></Field>}</div>)}<button className="text-button" onClick={() => onChange([...buttons, { id: newId('result_button'), text: 'Подробнее', action: 'branch' }])}><Plus size={14} /> Добавить кнопку</button></div>
}
...
function IntegrationsSection() {
  ...
  const [url, setUrl] = useState(initial.runtimeUrl)
  ...
  const save = async () => {
    setIntegrationConnection({ runtimeUrl: url, adminToken: token })
    try { setStatus(await saveYooKassa({ shopId, secretKey })); setSecretKey(''); setMessage('Реквизиты зашифрованы и сохранены в runtime.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Ошибка сохранения.') }
  }
  const check = async () => {
...
function BotSection({ document }: { document: FunnelDocument }) {
  ...
  return <><PageHeading eyebrow="Telegram + VK" title="Настройки бота" text="Здесь нет токенов и секретов — только поведение ботов и публичные имена." /><div className="settings-stack">
    <EditorGroup title="Основные параметры"><div className="form-grid"><Field label="Отображаемое название"><input value={bot.displayName} onChange={(event) => update((draft) => { draft.displayName = event.target.value })} /></Field><Field label="Telegram · Username бота"><div className="input-with-prefix"><span>@</span><input value={bot.username.replace(/^@/, '')} placeholder="my_funnel_bot" onChange={(event) => update((draft) => { draft.username = event.target.value.replace(/^@/, '') })} /></div></Field><Field label="VK · Сообщество"><input value={bot.vkCommunity ?? ''} placeholder="my_group или 123456789" onChange={(event) => update((draft) => { draft.vkCommunity = event.target.value || undefined })} /></Field><Field label="Часовой пояс по умолчанию"><input value={bot.timezone} onChange={(event) => update((draft) => { draft.timezone = event.target.value })} /></Field><Field label="Неактивность, дней"><input type="number" min="1" value={bot.inactivityDays} onChange={(event) => update((draft) => { draft.inactivityDays = Math.max(1, Number(event.target.value)) })} /></Field></div></EditorGroup>
    ...
    <EditorGroup title="Напоминания">
      <div className="form-grid">
        <Field label="Максимальное число"><input type="number" min="0" value={bot.reminders.maxCount} onChange={(event) => update((draft) => { draft.reminders.maxCount = Math.max(0, Number(event.target.value)) })} /></Field>
        <Toggle checked={bot.reminders.cancelAfterContinue} onChange={(cancelAfterContinue) => update((draft) => { draft.reminders.cancelAfterContinue = cancelAfterContinue })} label="Не отправлять после продолжения" />
        <Toggle checked={bot.reminders.respectQuietHours} onChange={(respectQuietHours) => update((draft) => { draft.reminders.respectQuietHours = respectQuietHours })} label="Учитывать тихие часы" />
      </div>
      <Field label="Текст незавершённого теста"><textarea rows={4} value={bot.reminders.testText} onChange={(event) => update((draft) => { draft.reminders.testText = event.target.value })} /></Field>
      <Field label="Текст незавершённого этапа"><textarea rows={4} value={bot.reminders.stageText} onChange={(event) => update((draft) => { draft.reminders.stageText = event.target.value })} /></Field>
    </EditorGroup>
    <EditorGroup title="Ссылки и источники" action={bot.trackingLinks.length ? <button className="button secondary" onClick={addLink}><Plus size={15} /> Создать ссылку</button> : undefined}><p className="panel-help">Ссылки помогают будущему боту сохранить источник пользователя и связать с ним результат, заявку и оплату.</p><div className="tracking-list">{bot.trackingLinks.map((link) => <TrackingLinkEditor key={link.id} document={document} link={link} />)}{!bot.trackingLinks.length && <Empty title="Ссылок пока нет" text="Создайте отслеживаемую ссылку для рекламы, публикации, канала или другого источника трафика." action={<button className="button secondary" onClick={addLink}><Plus size={15} /> Создать ссылку</button>} />}</div></EditorGroup>
  </div></>
}
...
function Empty({ title, text, action }: { title: string; text: string; action?: ReactNode }) { return <div className="entity-empty"><Beaker size={28} /><strong>{title}</strong><p>{text}</p>{action}</div> }
